...
try {
	$backupStartDate = (($logContent | select-string -Pattern "Heure de début|Start time") -split(":"))[1] -replace(" ","")
	if(!([string]::IsNullOrEmpty($backupStartDate))) {
	$backupStart = [datetime]::ParseExact($backupStartDate, "yyyyMMddHHmmss", $null)
	} else {
		if($Debug) { "No backupStartDate found" }
		$backupStart = ""
	}	
...
try {
	$backupEndDate = (($logContent | select-string -Pattern "Heure de fin|End time") -split(":"))[1] -replace(" ","")
	if(!([string]::IsNullOrEmpty($backupEndDate))) {
		$backupEnd = [datetime]::ParseExact($backupEndDate, "yyyyMMddHHmmss", $null)
	} else {
		if($Debug) { "No backupEndDate found" }
		$backupEnd = ""
	}
...
if(!([string]::IsNullOrEmpty($backupStartDate)) -and !([string]::IsNullOrEmpty($backupEndDate))) {
	try {
...
$(foreach($successfulComponent in $successfulComponents) { "`nSuccessfull backup : {0}" -f $successfulComponent })
$(foreach($failedComponent in $failedComponents) { "`nFailed backup : {0}" -f $failedComponent })
$(foreach($interruptedComponent in $interruptedComponents) { "`nInterrupted backup : {0}" -f $interruptedComponent })
...
$(if(!([string]::IsNullOrEmpty($backupDuration))) { "Backup duration : $($backupDuration.days) days, $($backupDuration.hours) hours, $($backupDuration.minutes) minutes" })
