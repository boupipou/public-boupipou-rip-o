...
$DSCImport = $ScriptLocation

#Reporting dependancies scripts and files
$M365DSCInformations = "Get-M365DSCInformations.json"

#Email and connection settings
if(Test-Path "$DSCImport\$M365DSCInformations") {
	$InformationsContent = Get-Content "$DSCImport\$M365DSCInformations" | ConvertFrom-Json
	if(!([string]::IsNullOrEmpty($InformationsContent))) {
		...
		"[ERROR] : Fail to retrieve file {0}\{1} content" -f $DSCImport,$M365DSCInformations
		Stop-Transcript
		exit
	}
} else {
	"[ERROR] : File {0}\{1} does not exist" -f $DSCImport,$M365DSCInformations
	Stop-Transcript
	exit
}
...
$ApplicationIdExport = $AppIdExport

#Azure AD application 'Write'
$ApplicationIdImport = $AppIdImport

$TranscriptFile = "$DSCImport\Transcript_Restoration_$FormattedDate.log"
...
switch($Help) {
	$true {
		...
		Write-Host "[INFO]: To perform a restoration from a file : .\PathToScript\$ScriptName`.ps1" -ForeGroundColor Gray -NoNewLine;Write-Host " -PathToDirectoryContainingRestorationFile DiskDrive:\Folder\" -ForeGroundColor Yellow
